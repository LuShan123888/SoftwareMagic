...
**弊端**：下面的场景就体现了 GOPATH 的弊端：项目A 和项B 依赖于某一 package 的不同版本 (分别为 `Pkg V1` 和 `Pkg V2` ) 。而 `src` 下只能允许一个版本存在，那项目A 和项B 就无法保证都能编译通过。
...
在 go.mod 文件图中，细心观察可以发现有些单元依赖带有 `+incompatible` 的后缀，这也是一个特殊标识符。对于 MAJOR 主版本在 V2 及以上的模块，go.mod 会在模块路径增加 `/vN` 后缀 (如下图中 `example/lib5/v3 v3.0.2` )。这能让 Go Module 按照不同的模块来处理同一个项目不同 MAJOR 主版本的依赖。
