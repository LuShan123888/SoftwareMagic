...
```bash
$ docker commit -m "haddop" -a "hadoop" fab4da838c2f newuhadoop
...
```bash
$ docker run -it --network hadoop -p 9870:9870 -p 8088:8088 \
--hostname "h01" \
--name "h01" \
newuhadoop /bin/bash
```
...
$ docker run -it --network hadoop -h "h04" --name "h04" newuhadoop /bin/bash
$ docker run -it --network hadoop -h "h05" --name "h05" newuhadoop /bin/bash
```
