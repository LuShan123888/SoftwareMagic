...
- 建表的同时通常还可以定义与该表有关的完整性约束条件,这些完整性约束条件被存入系统的数据字典中,当用户操作表中数据时由关系数据库管理系统自动检查该操作是否违背这些完整性约束条件.如果完整性约束条件设=涉及该表的多个属性列,则必须定义在表级上,否则既可以定义在列级,也可以定义在表级
- `AUTO_INCREMENT`:自增字段
- `COMMENT`:注释
- `DEFAULT`:默认值
...
不同的数据库产品在遵循SQL标准的基础上具体实现细节和处理策略会与标准有差别
