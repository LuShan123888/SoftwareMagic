...
```sql
SELECT Sno,Sname				3.最后在Student关系中取出Sno和
FROM Student					Sname
WHERE Sno IN
	(SELECT Sno					2.然后在SC关系中找出选修了3号课程
    	FROM SC					的学生学号
    	WHERE Cno IN
    		(SELECT Cno				1.首先在Course关系中找出"信息系统"
    			FORM Course			的课程号,结果为3号
    			WHERE Cname='信息系统';
    		)
    	);
```
...
- 执行内层,得到88(近似值),用该值代替内层查询,得到外层查询:
    ```sql
    SELECT Sno,Cno
    FROM SC x
    WHERE Grade>=88;
...
然后外层查询取出下一个元组重复做上述1-3步骤的处理,知道外层就的SC元组全部处理完毕,结果为:
...
WHERE EXISTS
	(SELECT *
	FROM SC
	 HERE Sno=Student.Sno AND Cno='1');
```
