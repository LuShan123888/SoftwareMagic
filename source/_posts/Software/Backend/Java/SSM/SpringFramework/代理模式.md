...
- 每个代理实例都有一个关联的调用处理程序1当在代理实例上调用方法时,方法调用将被编码并分派到其调用处理程序的`invock`方法
...
- 每个代理 都有一个关联的 用处理程序对象,它实现了接口InvocationHandler
...
```java
public static Object newProxyInstance( ClassLoader loader, 类<?> interfaces, InvocationHandler h) throws OllegalArgumentException
```
